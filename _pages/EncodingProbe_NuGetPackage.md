...
2026年6月6日に最初の preview1 をリリースし、若干のテスト・デバッグを進めて、2026年6月19日に preview2 をリリース、2026年6月23日 preview3 をリリースしました。

2026年7月2日 preview4 をリリースしました。

2026年7月8日 preview5 をリリースしました。
...
| bool          | UsePSName       | フレンドリ名が有効                  | PSEncodingName に有効なフレンドの名が入っている場合は True に、コードページが入っている場合は False となります。 |
...
PSEncodingName の値は、UsePSName = True の場合は、Get-Content 等の -Encoding で使用可能ですが、UserPSName = False の場合は、CodePage を使用してください。

UserPSName = False の場合、PSEncodingName もコードページを返しますが、文字列型として返すため、 -Encoding ではコードページとして認識してくれません。UserPSName = False の場合、は int型の CodePage を -Encoding オプションに渡す必要があります。

また、この仕様は PowerShell 6.2 以上の PowerShell にしか当てはまりません。Windows PowerShell 5.1 では必要なフレンドリ名が全て使用できないので、PSEncodingName も CodePage も使用できません。この対応は後のバージョンで対応する予定です。
...
注意点として、PSEncodingName のフレンドリ名は PowerShell 6.2 以上のバージョンに合わせて値を返します。Windows PowerShell 5.1 のフレンドリ名には対応していません。この対応は意外に複雑で、EncodingProbeの 1.0.0.0 では対応を見送ることにしました。1.1.0.0 で対応予定です。
...
text1.txt EncodingInformation { CodePage = 1200, EncodingName = utf-16, PSEncodingName = unicode, Bom = False, LineBreak = CrLf, IsWindowsOs = True, IsMacOs = False, IsLinuxOs = False, Culture = ja-JP }
