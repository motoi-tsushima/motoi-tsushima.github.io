...
preview5の UserPSName = False の場合に、CodePage を返す仕様は廃止して、EncodingWebName を返すようにしました。
...
注意点として、PowerShell 6.2 以上と Windows PowerShell 5.1 では、標準の文字エンコーディングのフレンドリ名は 異なる値が定義されています。

よって、PSEncodingName のフレンドリ名は、PowerShell 6.2 以上と Windows PowerShell 5.1 で異なるフレンドリ名を返します。
