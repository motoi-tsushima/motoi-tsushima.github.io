...
インストール方法の解説は、それぞれ以下のページで解説しています。

[rmsmf-txprobe インストール解説](/rmsmf_install_guide/)

rmsmf-txprobe は、旧.NET Framework 用なので、新規ユーザーには mfsr-mfprobe の利用を推奨します。

[mfsr-mfprobe インストール解説](/mfsr_install_guide/)

以前は、mfsr-mfprobeのインストールは、手動で実行ファイルをダウンロードして、インストールしていましたが、現在はパッケージマネージャを使用しているので、ダウンロードは必要ありません。

rmsmf-txprobe は、従来と同じで手動インストールが必要です。
