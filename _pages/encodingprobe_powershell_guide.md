...
両、Preview5 版をリリースしました。

EncodingInformation のOS情報を削除して、他の手段で情報提供する仕様に変更しました。

詳細解説は、保留にして、後に説明書を公開します。

Windows PowerShell 5.1 のテストを進めるうちに、PowerShell のバージョンにより文字エンコーディング名のフレンドリ名の仕様が大きく異なることが解り、その対応の為にいくつかの仕様変更と、リリース方針の変更を行いました。

Windows PowerShell 5.1 は、「BOM無しUTF-8」など必要な文字エンコーディング名のフレンドリ名が存在しません。utf8NoBOM などのフレンドリ名は、PowerShell 6.2 以降登場したもので、5.1 には存在しません。

そのため、Resolve-Encoding が PSEncodingName で返しているフレンドリ名が、PowerShell 5.1では使用できません。

また、PowerShell 5.1では、-Encoding で指定するフレンドリ名のデータ型も異なり、「BOM有りUTF-8」など使用できるフレンドリ名でも、PowerShell 7.6 と同じ物が使用できません。

PowerShell 5.1 への対応には、大がかりな工夫が必要で、1.0.0.0 のリリースでは見送ることにしました。
...
両、正式版の Version 1.0.0 をリリースしました。
